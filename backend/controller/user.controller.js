...
const toPublicUser = (user) => ({ id: user._id.toString(), name: user.name, email: user.email, role: user.role });
const createToken = (user) => jwt.sign({ id: user._id.toString(), email: user.email, role: user.role }, secret(), { expiresIn: "7d" });
...
export const registerUser = async (req, res) => {
  try {
    ...
    if (await User.exists({ email: normalizedEmail })) return res.status(409).json({ success: false, message: "User already exists" });
    const user = await User.create({ name: name.trim(), email: normalizedEmail, password: await bcrypt.hash(password, 12), role: "ADMIN" });
    return res.status(201).json({ success: true, user: toPublicUser(user), token: createToken(user) });
  } catch (error) {
...
export const loginUser = async (req, res) => {
  try {
    ...
    return res.json({ success: true, user: toPublicUser(user), token: createToken(user) });
  } catch (error) {
