...
/**
 * Shared dynamic Student ID Card preview.
 * Har student list (admin / franchise / teacher / batch / course)
 * se `student` object pass karo — card dynamic render hoga.
 */
export default function StudentIdCardModal({ student, loading, error, onClose }) {
  ...
  return (
    ...
    >
      <div className="id-modal-panel w-full max-w-4xl rounded-3xl bg-white shadow-2xl">
        <div className="id-controls flex items-center justify-between border-b border-slate-200 px-5 py-4 sm:px-7">
          <div>
            ...
            <p className="text-xs text-slate-500">Preview both sides before printing.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          student && (
            <>
              ...
              <div className="id-card-stage p-5 sm:p-8">
                <div className={`student-id-card ${side === "back" ? "is-back" : ""}`}>
                  <div className="id-brand">
                    <img src={logo} alt="AI Scholars" />
                    <div className="id-brand-lines" />
                  </div>
                    ...
                    <div className="id-back">
                      <div className="id-back-details">
                        <Details label="Course">
                          {student.courseId?.title || student.courseId?.name || student.course}
                        </Details>
                        <Details label="Gender">{student.gender}</Details>
                        <Details label="Blood Group">{student.bloodGroup}</Details>
                        <Details label="Father's Name">{student.fatherName}</Details>
                        <Details label="Address">{student.address}</Details>
                      </div>
                      <div className="id-signature">
                        <span>For AI SCHOLARS</span>
                        <div className="id-sign">Authorized</div>
                        <b>Auth. Signature</b>
                      </div>
                    </div>
                  )}
