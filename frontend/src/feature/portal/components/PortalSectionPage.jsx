...
const endpointFor = (type) =>
  ({
    ...
    fees: "/api/portal/fees",
  })[type];

export const PortalSectionPage = ({
  title,
  description,
  type,
  actionLabel,
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        {actionLabel && (
          <button
            ...
            className="rounded-xl bg-blue-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-blue-600/20 hover:bg-blue-700"
          >
            {actionLabel}
          </button>
        )}
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
          ...
          <span className="text-xs text-slate-400">Live backend data</span>
        </div>
        {loading ? (
              ...
              <thead className="bg-slate-50 text-xs uppercase tracking-wider text-slate-400">
                <tr>
                  {(type === "students"
                    ...
                    : type === "courses"
                      ? ["Course", "Level", "Duration", "Fee"]
                      : ["Student", "Course", "Total", "Pending"]
                  ).map((heading) => (
                  ...
                  <tr key={row._id} className="hover:bg-slate-50">
                    {type === "students" && (
                      <>
                        ...
                        <td className="px-5 py-4 text-slate-500">
                          {row.courseId?.title || "Not assigned"}
                        </td>
                    ...
                    {type === "courses" && (
                      <>
                        ...
                        <td className="px-5 py-4 text-slate-500">
                          {row.duration?.value} {row.duration?.unit}
                        </td>
                        <td className="px-5 py-4 font-bold text-slate-700">
                          ₹{Number(row.courseFee || 0).toLocaleString("en-IN")}
                        </td>
                      </>
                    )}
                    {type === "fees" && (
                      <>
                        ...
                        <td className="px-5 py-4 font-bold text-orange-600">
                          ₹
                          {Number(row.totalPending || 0).toLocaleString(
                            "en-IN",
                          )}
                        </td>
                      </>
