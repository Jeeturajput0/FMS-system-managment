    ...
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div>
          ...
          <h1 className="mt-2 text-2xl font-extrabold text-slate-900">{isEditMode ? "Edit Topic" : "Add Topic"}</h1>
          <p className="mt-1 text-sm text-slate-500">{isEditMode ? "Update this topic's content and placement." : "Create topics independently, then use them through their module."}</p>
        </div>

        {error && <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-600">{error}</div>}
        <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
          <form onSubmit={handleSubmit} className="space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center gap-3 border-b border-slate-100 pb-4">{isEditMode ? <Pencil className="h-5 w-5 text-orange-500" /> : <BookOpen className="h-5 w-5 text-orange-500" />}<h2 className="font-bold text-slate-900">Topic Information</h2></div>
            <label className="block text-sm font-bold text-slate-700">Module *<select value={moduleId} onChange={(event) => setModuleId(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-3 py-3 font-normal outline-none focus:border-orange-500"><option value="">Select module</option>{modules.map((module) => <option key={module._id} value={module._id}>{module.title}</option>)}</select></label>
            ...
            <button disabled={loading} className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-5 py-3 text-sm font-bold text-white hover:bg-orange-600 disabled:opacity-50">{isEditMode ? <Pencil className="h-4 w-4" /> : <Plus className="h-4 w-4" />}{loading ? "Saving..." : isEditMode ? "Save Topic" : "Add Topic"}</button>
          </form>

          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="font-bold text-slate-900">Topics in Selected Module</h2>{!moduleId ? <p className="mt-4 text-sm text-slate-400">Select a module to see topics.</p> : topics.length === 0 ? <p className="mt-4 text-sm text-slate-400">No topics added yet.</p> : <div className="mt-4 space-y-2">{topics.map((topic) => <button key={topic._id} onClick={() => navigate(`/admin/topics/${topic._id}`)} className="block w-full rounded-xl bg-slate-50 p-3 text-left text-sm font-semibold text-slate-700 hover:bg-orange-50 hover:text-orange-600">{topic.order}. {topic.title}</button>)}</div>}</section>
        </div>
      </div>
    </div>
  );
