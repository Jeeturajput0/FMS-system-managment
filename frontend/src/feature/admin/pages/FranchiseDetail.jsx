...
export const FranchiseDetail = () => {
  ...
  const franchiseStudents = students.filter(
    (s) =>
      s.franchise.toLowerCase().includes(franchise.name.toLowerCase()) ||
      s.franchiseId === franchise.id,
  );
  const franchisePayments = payments.filter((p) =>
    p.franchise.toLowerCase().includes(franchise.name.toLowerCase()),
  );
  ...
  return (
    <div className="space-y-6 pb-12">
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-xs">
                ...
                <div>
                  ...
                  <p className="font-semibold text-slate-900 mt-0.5">
                    {franchise.activeBatches} Batches
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-xs">
                ...
                <div>
                  ...
                  <p className="font-semibold text-slate-900 mt-0.5">
                    {franchise.teachersCount} Certified Instructors
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs space-y-4">
              ...
              <div className="space-y-2">
                {franchise.coursesOffered?.map((c, i) => (
                  <div
                    key={i}
                    className="p-2.5 rounded-xl bg-orange-50/50 border border-orange-100 text-xs font-semibold text-slate-800 flex items-center gap-2"
                  >
      ...
      {activeTab === "Students" && (
        <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs">
          <h3 className="font-bold text-slate-900 text-sm mb-4">
            Enrolled Students ({franchiseStudents.length})
          </h3>
      ...
      {activeTab === "Payments" && (
        <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs">
          <h3 className="font-bold text-slate-900 text-sm mb-4">
            Franchise Payments ({franchisePayments.length})
          </h3>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-600 uppercase font-semibold text-[10px]">
