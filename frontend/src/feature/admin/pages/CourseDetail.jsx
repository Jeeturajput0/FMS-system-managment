...
export const CourseDetail = () => {
  ...
  const enrolledList = students.filter(
    (s) =>
      s.courseId === (course.id || course._id) || s.course === course.title,
  );
        ...
        <div className="flex items-center gap-6 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6 shrink-0">
          <div>
            ...
            <p className="text-xl font-extrabold text-slate-900">
              {course.feePrice ||
                `Rs. ${Number(course.courseFee || 0).toLocaleString("en-IN")}`}
            </p>
          </div>
          <div>
            <p className="text-[10px] uppercase font-bold text-slate-600">
        ...
        {/* {isAdminView && (
          <div className="flex items-center gap-2">
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-3 py-2.5 text-sm font-bold text-white hover:bg-orange-600"
            >
              <Plus className="w-4 h-4" title="Add Module" />
            </Link>
          </div>
        )} */}
      </div>
        ...
        <div className="space-y-4">
          ...
          {course.modules && course.modules.length > 0 ? (
                      ...
                      <div className="p-4 divide-y divide-slate-100">
                        {module.topics?.map((topic) => (
                          <div
                            ...
                            className="py-3 flex items-center justify-between hover:bg-slate-50 px-3 rounded-xl transition-colors"
                          >
                            <div className="flex items-center gap-3">
                              {topic.type === "Video" && (
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-2 text-[10px] text-slate-600 font-medium mt-0.5">
                                  <span className="px-1.5 py-0.5 rounded-md bg-slate-100 font-semibold">
                                    {topic.type}
                                  </span>
                                  <span>•</span>
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-200 text-center text-xs text-slate-600 space-y-3">
              ...
              <p>
                Demo Syllabus Modules for this course are currently being mapped
                to AI Scholar LMS.
              </p>
            </div>
          )}
      ...
      {activeTab === "Students" && (
        <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs">
          <h3 className="font-bold text-slate-900 text-sm mb-4">
            Enrolled Candidates ({enrolledList.length})
          </h3>
