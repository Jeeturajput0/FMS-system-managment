...
const TeacherStudents = () => {
  ...
  const filtered = useMemo(
    () =>
      students.filter((s) =>
        `${s.name} ${s.email} ${s.courseId?.title} ${s.batchId?.name}`
          .toLowerCase()
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-black text-slate-900">My Students</h1>
        <p className="mt-2 text-sm text-slate-500">
          Students from your assigned courses and batches only.
        </p>
      </div>
      {error && (
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((s) => (
                <tr key={s._id} className="hover:bg-slate-50">
                  ...
                  <td className="p-4">{s.courseId?.title || "-"}</td>
        ...
        >
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {selected.email || "Student profile"}
                </p>
              </div>
              <button
                onClick={() => setSelected(null)}
            ...
            <div className="mt-5 grid gap-3 text-sm">
              <p>
                <b>Course:</b> {selected.courseId?.title || "-"}
              </p>
              <p>
                <b>Batch:</b> {selected.batchId?.name || "-"}
              </p>
              <p>
                <b>Status:</b> {selected.status || "active"}
              </p>
              <p>
                <b>Attendance:</b> {selected.attendancePercentage || 0}%
              </p>
            </div>
          </div>
        </div>
