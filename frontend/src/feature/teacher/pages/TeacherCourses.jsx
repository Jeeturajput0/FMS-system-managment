...
const TeacherCourses = () => {
  ...
  useEffect(() => {
    apiFetch("/api/portal/courses")
      ...
      .catch((requestError) =>
        setError(requestError.message || "Unable to load assigned courses"),
      )
  ...
  const filteredCourses = useMemo(() => {
    ...
    return courses.filter((course) =>
      [course.title, course.name, course.category, course.level].some((value) =>
        String(value || "")
          .toLowerCase()
          .includes(query),
      ),
  ...
  return (
    <section className="min-h-full space-y-5 bg-slate-50/40 p-1">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        <span className="text-xs font-bold text-slate-500">
          {filteredCourses.length} assigned course
            ...
            <table className="w-full min-w-[820px] text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {pageCourses.map((course) => (
                  <tr key={course._id} className="hover:bg-blue-50/30">
